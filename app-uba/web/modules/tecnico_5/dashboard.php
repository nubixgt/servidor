<?php
// web/modules/tecnico_5/dashboard.php
require_once '../../config/database.php';
require_once '../../config/workflow.php';
require_once '../../includes/verificar_sesion.php';

verificarRol('tecnico_5');

try {
    $database = new Database();
    $db = $database->getConnection();

    // Obtener denuncias para resolución final
    $etapas = WorkflowDenuncias::$etapasPorRol['tecnico_5'];
    ...
    // 3. Resueltas hoy por el usuario actual
    $hoy = date('Y-m-d');
    $sqlResueltasHoy = "SELECT COUNT(*) as total 
                        FROM seguimiento_denuncias 
                        WHERE procesado_por = :usuario_id 
                        AND DATE(fecha_procesamiento) = :hoy
                        AND etapa IN ('resolucion_final')
                        AND accion = 'resuelto'";
    $stmtResueltasHoy = $db->prepare($sqlResueltasHoy);
    $stmtResueltasHoy->bindParam(':usuario_id', $_SESSION['usuario_id']);
    $stmtResueltasHoy->bindParam(':hoy', $hoy);
    $stmtResueltasHoy->execute();
    $resueltasHoy = $stmtResueltasHoy->fetch(PDO::FETCH_ASSOC)['total'];

    // 4. Sin resolver (sin seguimiento en esta etapa)
    $sqlSinResolver = "SELECT COUNT(*) as total 
                       FROM denuncias d
                       WHERE d.estado_denuncia IN ('pendiente', 'en_proceso')
                       AND NOT EXISTS (
                           SELECT 1 FROM seguimiento_denuncias s 
                           WHERE s.id_denuncia = d.id_denuncia
                           AND s.etapa = 'resolucion_final'
                       )
                       AND COALESCE(
    ...
    $stmtSinResolver = $db->query($sqlSinResolver);
    $sinResolver = $stmtSinResolver->fetch(PDO::FETCH_ASSOC)['total'];
...
<head>
    ...
    <title>Resolución Final - Técnico 5 - AppUBA</title>
...
<body>
    <?php include '../../includes/navbar_tecnico5.php'; ?>

    <div class="dashboard-container">
        <!-- Header -->
        <div class="welcome-section">
            <h1><i class="fas fa-stamp"></i> Mi Área: Resolución Final</h1>
            <p class="subtitle">Resoluciones finales y cierre de expedientes</p>
        </div>
                    ...
                    <?php foreach ($denuncias as $denuncia): ?>
                        <tr>
                            ...
                            <td>
                                ...
                                <a href="/app-uba/web/modules/admin/resolucion_final/procesar.php?id=<?php echo $denuncia['id_denuncia']; ?>"
    ...
    <!-- JS personalizado -->
    <script src="../../js/dashboard_tecnico5.js"></script>
    ...
    <script>
        ...
        function verDetalleDenuncia(id) {
            Swal.fire({
                ...
                window.location.href = '/app-uba/web/modules/admin/resolucion_final/detalle_denuncia.php?id=' + id;
            });
        }
    </script>
