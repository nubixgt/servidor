 ...
 * INVENTARIO - JavaScript con SweetAlert2 + NUEVOS CAMPOS
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  initSelect2();
  initFormValidation();
  initCostoFormatting(); // ⭐ NUEVO: Formateo de costo
});

/**
 * ⭐ NUEVO: Inicializar formateo de costo
 */
function initCostoFormatting() {
  const costoInput = document.getElementById("costo_equipo");

  if (!costoInput) return;

  // Evento input: solo validar números y punto decimal (SIN formatear)
  ...
  // Evento blur: formatear a Q0,000.00
  costoInput.addEventListener("blur", function (e) {
    formatearCostoFinal(this);
  });

  // Evento focus: quitar formato para editar
  costoInput.addEventListener("focus", function (e) {
    let valor = this.value;

    // Quitar Q, comas y espacios
  ...
  console.log("✅ Formateo de costo inicializado");
}

/**
 * ⭐ NUEVO: Formatear costo al salir del campo
 */
function formatearCostoFinal(input) {
...
/**
 * ⭐ NUEVO: Limpiar formato de costo para enviar a BD
 */
function limpiarCosto(valorFormateado) {
  if (!valorFormateado) return null;

  // Remover Q, comas y espacios
  let valor = valorFormateado.replace(/[Q,\s]/g, "");
    ...
    reader.onload = function (e) {
      ...
      if (file.type === "application/pdf") {
        previewItem.innerHTML = `
                    ...
                    <button type="button" class="remove-image" onclick="removerImagen(${
...
function guardarEquipo() {
  ...
  // ⭐ NUEVO: Agregar cantidad
  formData.append("cantidad", document.getElementById("cantidad").value || "1");

  // ⭐ NUEVO: Limpiar y agregar costo
  const costoFormateado = document.getElementById("costo_equipo").value;
  const costoLimpio = limpiarCosto(costoFormateado);
  formData.append("costo_equipo", costoLimpio || "");

  // ⭐ NUEVO: Agregar proveedor y fecha de compra
  formData.append(
    "proveedor_id",
    document.getElementById("proveedor_id").value || ""
  );
  ...
  btnGuardar.innerHTML =
    '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="animation: spin 1s linear infinite;"><circle cx="12" cy="12" r="10"></circle><path d="M12 6v6l4 2"></path></svg><span>Guardando...</span>';
    ...
    .then((data) => {
      if (data.success) {
        ...
        if (fotos.length > 0) {
          ...
          fotosHTML =
            '<p style="color: #9ca3af; font-style: italic;">Sin fotografías</p>';
        }
        ...
        // ⭐ NUEVO: Formatear costo para mostrar
        let costoHTML =
          '<p style="color: #9ca3af; font-style: italic;">Sin costo registrado</p>';
        if (equipo.costo_equipo) {
          const costoFormateado =
            "Q" +
            parseFloat(equipo.costo_equipo).toLocaleString("es-GT", {
              minimumFractionDigits: 2,
              maximumFractionDigits: 2,
            });
          costoHTML = `<div style="font-size: 16px; color: #065f46; font-weight: 600;">${costoFormateado}</div>`;
        }

        // ⭐ NUEVO: Mostrar cantidad
        let cantidadHTML =
          '<p style="color: #9ca3af; font-style: italic;">Sin cantidad registrada</p>';
        if (equipo.cantidad) {
          cantidadHTML = `<div style="font-size: 16px; color: #7c3aed; font-weight: 600;">${equipo.cantidad} unidades</div>`;
        }

        // ⭐ NUEVO: Mostrar fecha de compra
        let fechaCompraHTML =
          '<p style="color: #9ca3af; font-style: italic;">Sin fecha registrada</p>';
        if (equipo.fecha_compra) {
          const fecha = new Date(equipo.fecha_compra + "T00:00:00");
          const opciones = { year: "numeric", month: "long", day: "numeric" };
          fechaCompraHTML = `<div style="font-size: 14px; color: #1e3a8a; font-weight: 500;">${fecha.toLocaleDateString(
            "es-GT",
            opciones
          )}</div>`;
        }

        // ⭐ NUEVO: Obtener nombre del proveedor desde la API
        let proveedorHTML =
          '<p style="color: #9ca3af; font-style: italic;">Sin proveedor</p>';
        if (equipo.proveedor_nombre) {
          proveedorHTML = `<div style="font-size: 14px; color: #5b21b6; font-weight: 500;">${equipo.proveedor_nombre}</div>`;
        }
                        ...
                        <div style="text-align: left; padding: 20px;">
                            <div style="display: grid; gap: 16px;">
                                <div style="padding: 12px; background: linear-gradient(135deg, #fee2e2 0%, #fecaca 100%); border-radius: 10px; border-left: 4px solid #dc2626;">
                                    ...
                                    <div style="font-size: 16px; color: #7f1d1d; font-weight: 600;">${
                                      equipo.tipo_equipo
                                    }</div>
                                </div>
                                
                                <div style="padding: 12px; background: linear-gradient(135deg, #d1fae5 0%, #6ee7b7 100%); border-radius: 10px; border-left: 4px solid #10b981;">
                                    <div style="font-size: 11px; color: #065f46; font-weight: 700; text-transform: uppercase; margin-bottom: 4px;">💰 Costo del Equipo</div>
                                    ${costoHTML}
                                </div>
                                
                                <div style="padding: 12px; background: linear-gradient(135deg, #ede9fe 0%, #ddd6fe 100%); border-radius: 10px; border-left: 4px solid #8b5cf6;">
                                    <div style="font-size: 11px; color: #7c3aed; font-weight: 700; text-transform: uppercase; margin-bottom: 4px;">📦 Cantidad</div>
                                    ${cantidadHTML}
                                </div>
                                
                                <div style="padding: 12px; background: linear-gradient(135deg, #ede9fe 0%, #ddd6fe 100%); border-radius: 10px; border-left: 4px solid #8b5cf6;">
                                    <div style="font-size: 11px; color: #7c3aed; font-weight: 700; text-transform: uppercase; margin-bottom: 4px;">🏪 Proveedor</div>
                                    ${proveedorHTML}
                                </div>
                                ...
                                <div style="padding: 12px; background: linear-gradient(135deg, #dbeafe 0%, #bfdbfe 100%); border-radius: 10px; border-left: 4px solid #3b82f6;">
                                    <div style="font-size: 11px; color: #1e40af; font-weight: 700; text-transform: uppercase; margin-bottom: 4px;">📅 Fecha de Compra</div>
                                    ${fechaCompraHTML}
                                </div>
                                ...
                                <div style="padding: 12px; background: linear-gradient(135deg, #fef3c7 0%, #fde68a 100%); border-radius: 10px; border-left: 4px solid #f59e0b;">
                                    ...
                                    <div style="font-size: 14px; color: #78350f; font-weight: 500;">${
                                      equipo.proyecto_nombre || "Sin asignar"
                                    }</div>
                                </div>
                                ...
                                <div style="padding: 12px; background: linear-gradient(135deg, #dbeafe 0%, #bfdbfe 100%); border-radius: 10px; border-left: 4px solid #3b82f6;">
                                    ...
                                    <div style="font-size: 14px; color: #1e3a8a; font-weight: 500;">${
                                      equipo.contratista_nombre || "Sin asignar"
                                    }</div>
                                </div>
                                
                                <div style="padding: 12px; background: linear-gradient(135deg, #ffedd5 0%, #fed7aa 100%); border-radius: 10px; border-left: 4px solid ${
  ...
  fetch(`${BASE_URL}/api/inventario.php?id=${id}`)
    ...
    .then((data) => {
      if (data.success) {
        ...
        // ⭐ NUEVO: Cargar cantidad
        document.getElementById("cantidad").value = equipo.cantidad || "1";

        // ⭐ NUEVO: Formatear costo para edición
        if (equipo.costo_equipo) {
          const costoFormateado =
            "Q" +
            parseFloat(equipo.costo_equipo).toLocaleString("es-GT", {
          ...
          document.getElementById("costo_equipo").value = costoFormateado;
        } else {
          document.getElementById("costo_equipo").value = "";
        }

        // ⭐ NUEVO: Cargar proveedor y fecha de compra
        document.getElementById("proveedor_id").value =
          equipo.proveedor_id || "";
        document.getElementById("fecha_compra").value =
          equipo.fecha_compra || "";
        ...
        jQuery("#proyecto_id")
          .val(equipo.proyecto_id || "")
          .trigger("change");
        jQuery("#contratista_id")
          .val(equipo.contratista_id || "")
          .trigger("change");
