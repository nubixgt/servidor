    ...
    <div class="glass-card overflow-hidden">
      ...
      <form @submit.prevent="handleSubmit" class="p-6 sm:p-8 space-y-6">
        ...
        <div class="pt-6 border-t border-outline-variant/20 flex justify-end gap-3">
          <button type="button" @click="router.push('/tech')" class="px-6 py-3 bg-[var(--color-surface-container-low)] text-on-surface-variant rounded-xl font-medium hover:bg-[var(--color-surface-container)] transition-colors">
            Cancelar
          </button>
...
const type = ref(route.path.includes('egreso') ? 'egreso' : 'ingreso');
...
const handleSubmit = async () => {
  ...
  try {
    ...
    form.value = { amount: '', transaction_date: new Date().toISOString().split('T')[0], category: '', provider: '', description: '' };
