  ...
  <div class="flex h-screen overflow-hidden bg-surface font-body antialiased relative">
    ...
    <aside class="w-64 h-full glass-panel shadow-sm flex flex-col z-20 relative">
        ...
        <div v-if="isAdmin">
          ...
          <div class="space-y-1">
            <router-link
              ...
              :class="route.path && route.path.startsWith(link.path) ? 'bg-primary-fixed/40 text-primary font-bold' : 'text-on-surface-variant hover:bg-surface-container-low'"
            >
              <div v-show="route.path && route.path.startsWith(link.path)" class="absolute left-0 top-0 bottom-0 w-1 bg-primary rounded-r-full"></div>
              <span class="material-symbols-outlined" :class="{'filled': route.path && route.path.startsWith(link.path)}">{{ link.icon }}</span>
        ...
        <div v-else>
          ...
          <div class="space-y-1">
            <router-link
              ...
              :to="link.path"
              class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors relative group"
              :class="route.path === link.path ? 'bg-primary-fixed/40 text-primary font-bold' : 'text-on-surface-variant hover:bg-surface-container-low'"
            >
              <div v-show="route.path === link.path" class="absolute left-0 top-0 bottom-0 w-1 bg-primary rounded-r-full"></div>
              <span class="material-symbols-outlined" :class="{'filled': route.path === link.path}">{{ link.icon }}</span>
              <span class="text-sm">{{ link.name }}</span>
      ...
      <div class="p-4 mt-auto border-t border-outline-variant/20">
        <button @click="handleLogout" class="w-full flex items-center gap-3 p-2 rounded-xl hover:bg-surface-container-low transition-colors cursor-pointer text-left">
