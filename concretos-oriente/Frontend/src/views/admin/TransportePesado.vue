    ...
    <template v-if="activeTab === 'fleet'">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        <div v-for="u in filteredList" :key="u.id"
          ...
          <div
            v-if="u.foto_delantera"
            class="rounded-2xl overflow-hidden aspect-video bg-black/20 border border-white/5 cursor-zoom-in relative group/cardphoto"
            @click="openImagePreview(photoUrl(u.foto_delantera), `${u.marca || ''} ${u.modelo || ''} - Foto Delantera`)"
          >
            <img :src="photoUrl(u.foto_delantera)" class="w-full h-full object-cover group-hover/cardphoto:scale-105 transition-transform duration-300" />
              ...
              <div class="grid grid-cols-2 gap-3">
                <div v-for="photo in photoFields" :key="photo.key" class="space-y-1">
                  ...
                  <div
                    ...
                    :class="selectedUnit[photo.key] ? 'cursor-zoom-in hover:border-primary/50' : ''"
                    @click="selectedUnit[photo.key] && openImagePreview(photoUrl(selectedUnit[photo.key]), `${selectedUnit.marca || ''} ${selectedUnit.modelo || ''} - Foto ${photo.label}`)"
                  >
                    <img v-if="selectedUnit[photo.key]" :src="photoUrl(selectedUnit[photo.key])" class="w-full h-full object-cover group-hover/photo:scale-105 transition-transform duration-300" />
                    <div v-if="selectedUnit[photo.key]" class="absolute inset-0 bg-black/40 opacity-0 group-hover/photo:opacity-100 transition-opacity flex items-center justify-center">
                      <span class="px-2.5 py-1 rounded-lg bg-black/60 backdrop-blur-sm text-[10px] font-bold text-white uppercase tracking-wider flex items-center gap-1 border border-white/20">
                        <EyeIcon class="w-3.5 h-3.5 text-primary" /> Ampliar
                      </span>
                    </div>
...
const startEdit = (u) => {
  ...
  photoPreviews.value = {
    ...
    foto_trasera:   u.foto_trasera   ? photoUrl(u.foto_trasera)   : null,
    foto_lateral1:  u.foto_lateral1  ? photoUrl(u.foto_lateral1)  : null,
    foto_lateral2:  u.foto_lateral2  ? photoUrl(u.foto_lateral2)  : null,
  };
