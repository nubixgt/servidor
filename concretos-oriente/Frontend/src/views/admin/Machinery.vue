          ...
          <div>
            ...
            <div class="p-6 space-y-4">
              ...
              <div class="bg-black/20 rounded-2xl p-4 border border-white/5 space-y-2 text-xs">
                ...
                <div v-if="m.clasificacion_tipo === 'Liviana'" class="flex items-center justify-between">
                  <span class="text-[9px] font-black text-amber-400/70 uppercase tracking-widest">Frec. Servicio</span>
                  <span class="font-black text-amber-300">{{ m.frecuencia_servicio_horas }} hrs</span>
                </div>
...
<div class="space-y-6">
  ...
  <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
    <div>
      <h2 class="text-3xl font-black italic tracking-tighter uppercase text-white">Gestión de Maquinaria</h2>
      <p class="text-xs font-bold text-white/40 uppercase tracking-widest mt-0.5">Control de flota, bitácoras y mantenimientos</p>
    </div>

    <div class="flex items-center gap-3">
      <button 
        @click="switchTab('machinery')"
        :class="activeTab === 'machinery' ? 'bg-primary text-white shadow-lg shadow-primary/30' : 'bg-white/5 text-white/60 hover:bg-white/10'"
        class="px-5 py-3 rounded-2xl text-xs font-black uppercase tracking-wider transition-all flex items-center gap-2 border border-white/10"
      >
        <WrenchScrewdriverIcon class="w-4 h-4" /> Flota de Maquinaria
      </button>
      <button 
        @click="switchTab('log')"
        :class="activeTab === 'log' ? 'bg-primary text-white shadow-lg shadow-primary/30' : 'bg-white/5 text-white/60 hover:bg-white/10'"
        class="px-5 py-3 rounded-2xl text-xs font-black uppercase tracking-wider transition-all flex items-center gap-2 border border-white/10"
      >
        <ClockIcon class="w-4 h-4" /> Bitácora Diaria
      </button>
      <button 
        @click="startRegister"
        class="px-5 py-3 bg-white/10 hover:bg-white/20 text-white rounded-2xl text-xs font-black uppercase tracking-wider transition-all flex items-center gap-2 border border-white/20 shadow-lg"
      >
        <PlusIcon class="w-4 h-4" /> Nueva Maquina
      </button>
    </div>
  </div>
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
    <div v-for="(kpi, i) in metrics" :key="i" class="glass-card p-6 rounded-[28px] border border-white/10 relative overflow-hidden group">
      <div class="absolute -right-6 -bottom-6 w-24 h-24 bg-white/5 rounded-full blur-2xl group-hover:bg-primary/20 transition-all"></div>
      <div class="flex items-center justify-between mb-4">
        <span class="text-[10px] font-black uppercase tracking-[0.2em] text-white/40">{{ kpi.label }}</span>
        <component :is="kpi.icon" :class="['w-5 h-5', kpi.color]" />
      </div>
      <div class="flex items-baseline gap-2">
        <h3 class="text-3xl font-black italic tracking-tighter uppercase text-white">{{ kpi.value }}</h3>
        <span v-if="kpi.percentage !== undefined" class="text-xs font-black text-primary">{{ kpi.percentage }}%</span>
      </div>
      <p class="text-[10px] font-bold text-white/30 uppercase tracking-wider mt-1">{{ kpi.trend }}</p>
    </div>
  </div>

  <!-- ═══════════════════════════════════════════ TAB: FLOTA DE MAQUINARIA ═══════════════════════════════════════════ -->
  <template v-if="activeTab === 'machinery'">
    <div class="glass-card p-6 rounded-[32px] border border-white/10 flex flex-col md:flex-row gap-4 items-center justify-between">
      <div class="relative w-full md:w-96">
        <MagnifyingGlassIcon class="w-4 h-4 absolute left-4 top-1/2 -translate-y-1/2 text-white/40" />
        <input 
          v-model="searchMachine" 
          type="text" 
          placeholder="Buscar por código, marca, modelo, serie, factura..." 
          class="w-full bg-black/20 border border-white/10 rounded-2xl pl-11 pr-4 py-3 text-xs font-bold text-white placeholder-white/30 focus:outline-none focus:border-primary/50 transition-all"
        />
      </div>

      <div class="flex items-center gap-3 w-full md:w-auto">
        <select v-model="filterType" class="bg-black/20 border border-white/10 rounded-2xl px-4 py-3 text-xs font-bold text-white focus:outline-none focus:border-primary/50 appearance-none min-w-[140px]">
          <option value="">Todas las Clases</option>
          <option value="Pesada">Pesada</option>
          <option value="Liviana">Liviana</option>
        </select>
        <select v-model="filterStatus" class="bg-black/20 border border-white/10 rounded-2xl px-4 py-3 text-xs font-bold text-white focus:outline-none focus:border-primary/50 appearance-none min-w-[140px]">
          <option value="">Todos los Estados</option>
    ...
    <div v-if="loading" class="text-center py-20 text-white/50 text-xs font-bold">Cargando flota de maquinaria...</div>
    <div v-else-if="filteredMachinery.length === 0" class="glass-card p-16 rounded-[32px] text-center border border-white/10">
      <WrenchScrewdriverIcon class="w-12 h-12 mx-auto text-white/20 mb-4" />
      <h3 class="text-lg font-black uppercase text-white tracking-tight">No se encontró maquinaria</h3>
      <p class="text-xs text-white/40 mt-1 uppercase tracking-wider">Intenta con otros filtros de búsqueda</p>
    </div>
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div v-for="m in filteredMachinery" :key="m.id" class="glass-card rounded-[32px] border border-white/10 overflow-hidden flex flex-col justify-between group hover:border-primary/50 transition-all shadow-xl">
        
        <!-- Card Header Image -->
        <div class="relative h-48 bg-black/40 overflow-hidden">
          <img v-if="m.foto_path" :src="getPhotoUrl(m.foto_path)" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
          <div v-else class="w-full h-full flex items-center justify-center text-white/10 bg-white/[0.02]">
            <WrenchScrewdriverIcon class="w-16 h-16" />
          </div>
          <div class="absolute inset-0 bg-gradient-to-t from-[#0B0F19] via-transparent to-transparent"></div>
          
          <!-- Badge Estado -->
          <div class="absolute top-4 right-4 flex items-center gap-1.5 px-3 py-1 rounded-xl bg-black/60 backdrop-blur-md border border-white/10 text-[10px] font-black uppercase tracking-wider text-white">
            <div :class="`w-2 h-2 rounded-full ${getStatusColor(m.estado)}`"></div>
            {{ m.estado }}
          </div>
          ...
          <div class="absolute top-4 left-4">
            <span :class="[
              'px-2.5 py-1 rounded-xl text-[10px] font-black uppercase tracking-wider backdrop-blur-md border',
              m.clasificacion_tipo === 'Liviana' ? 'bg-amber-500/20 border-amber-500/30 text-amber-300' : 'bg-primary/20 border-primary/30 text-primary'
            ]">
              {{ m.clasificacion_tipo || 'Pesada' }}
            </span>
          </div>
  ...
  <template v-else-if="activeTab === 'register'">
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
      ...
      <div class="lg:col-span-8 space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div class="space-y-2">
              ...
              <input 
                ...
                required
                placeholder="Ej. Rotomartillo, Retro, Sapo..."
        ...
        <!-- Sección de Proveedor y Adquisición -->
        <section class="glass-card p-8 rounded-3xl border border-white/5">
          <h3 class="text-xs font-black uppercase tracking-widest text-primary mb-6 flex items-center gap-2">
            <BuildingOfficeIcon class="w-4 h-4" /> Datos de Adquisición
          </h3>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div class="space-y-2">
              <label class="text-[9px] font-black text-white/30 uppercase tracking-widest">Costo de Adquisición (Q)</label>
              <input 
            ...
            <div class="space-y-2">
              <label class="text-[9px] font-black text-white/30 uppercase tracking-widest">Proveedor de Compra</label>
              <input 
                v-model="formMachine.proveedor_compra" 
                type="text" 
            ...
            <div class="space-y-2">
              <label class="text-[9px] font-black text-white/30 uppercase tracking-widest">Teléfono de Proveedor</label>
              <input 
                v-model="formMachine.telefono_proveedor" 
                type="text" 
    ...
    <div class="glass-card w-full max-w-2xl overflow-y-auto rounded-[32px] p-6 md:p-8 relative z-10 border border-white/10 shadow-2xl">
      ...
      <form @submit.prevent="submitLog" class="space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div class="space-y-2 md:col-span-2">
            <label class="text-xs font-bold text-white/50 uppercase tracking-wider">Precio de Renta Diario (Q)</label>
            <input v-model="formLog.precio_renta" type="number" step="0.01" placeholder="Ej. 1500.00" class="w-full bg-black/20 border border-white/10 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-primary/50" />
          </div>
        ...
        <div class="lg:w-1/2 p-8 lg:p-10 bg-black/20 overflow-y-auto space-y-6">
          ...
          <div>
            ...
            <div class="grid grid-cols-2 gap-3 bg-black/20 p-4 rounded-2xl border border-white/5">
              ...
              <div v-if="selectedMachine.clasificacion_tipo === 'Liviana'"><p class="text-[9px] text-amber-400 uppercase tracking-widest">Frec. de Servicio (Hrs)</p><p class="text-xs font-bold text-amber-300">{{ selectedMachine.frecuencia_servicio_horas || '0' }}</p></div>
...
const openEditMachine = (m) => {
  formMachine.value = { 
    ...
    frecuencia_servicio_horas: m.frecuencia_servicio_horas || 0,
    proveedor_compra: m.proveedor_compra || '',
...
const openLogModal = () => {
  formLog.value = {
    ...
    combustible_consumido: '', observaciones: '', precio_renta: 0
  };
