<script setup>
import { ref } from 'vue';
import { MessageCircle, X, Send, CheckCheck, Shield } from 'lucide-vue-next';
...
const messages = ref([
  {
    sender: 'bot',
    text: '¡Hola! 👋 Bienvenido a CoreFix Services. ¿En qué podemos ayudarte hoy?',
    time: 'Justo ahora'
  }
]);
...
const handleSend = (textToSend = '') => {
  const text = textToSend || inputMsg.value;
  if (!text.trim()) return;
...
const handleOpenDirectWhatsApp = () => {
  const defaultText = encodeURIComponent('Hola CoreFix Services, quisiera consultar por una reparación de mi equipo.');
...
<template>
  ...
  <div class="fixed bottom-6 right-6 z-50 flex flex-col items-end">
    ...
    <!-- Chat Popover Window -->
    <div v-if="isOpen" class="mb-3 w-80 sm:w-96 bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden animate-in slide-in-from-bottom-5 duration-200">
      <!-- Header -->
      <div class="bg-[#075e54] text-white p-4 flex items-center justify-between">
        <div class="flex items-center gap-3">
          <div class="relative">
            <div class="w-10 h-10 rounded-full bg-emerald-500 flex items-center justify-center font-bold text-white shadow-inner">
              TF
            </div>
            <span class="absolute bottom-0 right-0 w-3 h-3 bg-emerald-400 border-2 border-[#075e54] rounded-full" />
          </div>
          <div>
            <h4 class="font-semibold text-sm">CoreFix Soporte</h4>
      ...
      <!-- Chat Body -->
      <div class="p-4 bg-[#efeae2] h-72 overflow-y-auto space-y-3 text-xs">
        <div class="bg-amber-50 text-amber-900 border border-amber-200 p-2 rounded-lg text-[11px] flex items-start gap-1.5 shadow-xs">
          <Shield class="w-3.5 h-3.5 text-amber-700 shrink-0 mt-0.5" />
        ...
        >
          <div
            class="max-w-[82%] rounded-xl p-2.5 shadow-xs"
            :class="m.sender === 'user'
          ...
          >
            ...
            <div class="flex items-center justify-end gap-1 mt-1 text-[10px] text-slate-400">
              <span>{{ m.time }}</span>
              <CheckCheck v-if="m.sender === 'user'" class="w-3 h-3 text-blue-500" />
            </div>
          </div>
        </div>

        <!-- Quick suggestions -->
        <div v-if="messages.length <= 2" class="pt-2 space-y-1.5">
          <p class="text-[10px] text-slate-500 font-medium">Consultas frecuentes:</p>
          <button
            v-for="(opt, i) in quickOptions"
            :key="i"
            @click="handleSend(opt)"
            class="block w-full text-left bg-white/90 hover:bg-white text-slate-700 p-2 rounded-lg border border-slate-200/80 text-[11px] transition-colors shadow-2xs cursor-pointer"
          >
            {{ opt }}
          </button>
        </div>
      ...
      <div class="p-3 bg-white border-t border-slate-100 space-y-2">
        <form
        ...
        >
          <input
            ...
            class="flex-1 text-xs bg-slate-100 rounded-xl px-3 py-2 border-none focus:outline-none focus:ring-2 focus:ring-emerald-500 text-slate-800"
          />
          <button
            type="submit"
            class="bg-[#25D366] hover:bg-[#20bd5a] text-white p-2 rounded-xl transition-colors cursor-pointer"
        ...
        <button
          @click="handleOpenDirectWhatsApp"
          class="w-full text-center py-1.5 text-[11px] font-semibold text-emerald-700 hover:text-emerald-800 hover:underline flex items-center justify-center gap-1 cursor-pointer"
        >
    ...
    <button
      ...
      class="w-14 h-14 rounded-full bg-[#25D366] hover:bg-[#20bd5a] text-white flex items-center justify-center shadow-lg hover:shadow-xl hover:scale-105 active:scale-95 transition-all duration-200 cursor-pointer group relative"
