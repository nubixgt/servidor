        ...
        <div v-else class="space-y-6">
            ...
            <div class="border-b-4 border-slate-900 pb-4 flex justify-between items-center">
                ...
                <div class="text-right">
                    <h2 class="text-sm font-sans font-extrabold uppercase text-slate-800">Acta de Supervisión</h2>
                    <p class="font-mono text-xs font-bold text-slate-900">Nº ACTA-SUP-{{ supervision.id.toString().padStart(5, '0') }}</p>
            ...
            <div class="grid grid-cols-2 gap-6 bg-slate-50 p-4 border border-slate-200 rounded-xl font-sans text-xs">
                <div>
                    ...
                    <p class="text-slate-600 mt-1">Fecha de Supervisión: <span class="font-bold text-slate-900">{{ supervision.fecha_supervision }}</span></p>
                </div>
                <div class="border-l border-slate-200 pl-6">
                    <h3 class="font-extrabold text-[10px] uppercase text-slate-500 tracking-wider mb-2">2. Inspector Responsable</h3>
                    <p class="text-sm font-bold text-slate-900">{{ supervision.inspector_nombre }}</p>
                    <p class="text-slate-600 mt-1">Código: <span class="font-mono font-bold">{{ supervision.inspector_codigo }}</span> | Área: <span class="font-bold">{{ supervision.inspector_area }}</span></p>
                </div>
            </div>
            ...
            <div class="space-y-6">
                <!-- Hallazgos -->
                <div>
                    <h3 class="font-sans font-extrabold text-xs uppercase text-slate-800 tracking-wider border-b border-slate-300 pb-1 mb-2">3. Hallazgos Sanitarios Detectados</h3>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-4 text-xs font-sans mt-2">
                        ...
                        <div v-if="supervision.fecha_cumplimiento">
                            <span class="text-[10px] text-slate-500 block uppercase font-bold">Fecha Cumplimiento</span>
                            <span class="text-xs font-bold text-slate-900 mt-1 block">{{ supervision.fecha_cumplimiento }}</span>
                        </div>
                    </div>
            ...
            <div class="pt-20">
                <div class="grid grid-cols-2 gap-12 font-sans text-xs">
                    <div class="text-center">
                        ...
                        <p class="font-bold text-slate-900">{{ supervision.inspector_nombre }}</p>
                        <p class="text-[10px] text-slate-500">Inspector Responsable (Firma)</p>
                        <p class="text-[9px] text-slate-400 font-mono mt-0.5">Código: {{ supervision.inspector_codigo }}</p>
                    </div>
                    <div class="text-center">
                        <div class="border-t border-slate-400 mx-auto w-48 mb-2"></div>
                        <p class="font-bold text-slate-900">Representante del Establecimiento</p>
                        <p class="text-[10px] text-slate-500">Responsable / Encargado (Firma)</p>
