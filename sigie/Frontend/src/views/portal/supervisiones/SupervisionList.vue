<template>
    <div>
        ...
        <div class="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-extrabold tracking-tight text-white font-headline">Supervisiones a Establecimientos</h1>
                <p class="text-xs text-white/60 mt-1">Historial y seguimiento de supervisiones a establecimientos registradas.</p>
            </div>
        ...
        <div class="glass-card backdrop-blur-sm rounded-2xl border border-white/20 shadow-premium overflow-hidden">
            ...
            <div v-else-if="filteredSupervisiones.length === 0" class="py-20 text-center">
                <span class="material-symbols-outlined text-5xl text-slate-400">store</span>
                <p class="text-sm font-semibold text-white mt-4">No se encontraron supervisiones registradas</p>
            ...
            <div v-else class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-200 text-xs">
                        <tr v-for="s in filteredSupervisiones" :key="s.id" class="hover:bg-black/20 transition-colors">
                            <!-- Establishment -->
                            <td class="px-6 py-4">
                                <p class="font-bold text-white text-sm">{{ s.establecimiento }}</p>
                                <p class="text-[9px] text-slate-400 font-mono">ID: {{ s.id }}</p>
                            </td>

                            <!-- Inspector -->
                            <td class="px-6 py-4">
                                <p class="font-bold text-white">{{ s.inspector_nombre }}</p>
                                <p class="text-[9px] text-slate-400 font-mono">Código: {{ s.inspector_codigo }}</p>
                            </td>
                            ...
                            <td class="px-6 py-4 text-slate-400">
                                <span v-if="s.norma_especifica" class="font-semibold text-gray-300 bg-slate-100 px-2 py-1 rounded border border-white/10">
                                    {{ s.norma_especifica }}
                                </span>
